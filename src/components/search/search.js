...
class Search extends Component {
  ...
  searchPlates() {
    ...
    this.setState({ searchResults: [], selectedSearchResult: null }, () => {
      openDBRequest.onsuccess = (e) => {
        ...
        openRequest.onsuccess = (e) => {
          ...
          if (cursor) {
            ...
            // const searchedPlates = cursor.value.plate.find(plate => colleague.id === impersonation.relationships.target.data.id);
            ...
            if (cursor.value.plate.indexOf(searchTerm) !== -1) {
              ...
              this.setState({
                searchResults: [...this.state.searchResults, cursor.value]
              });
                      ...
                      <Table className={classes.table}>
                        <TableHead>
                          <TableRow>
                            ...
                            <CustomTableCell padding="dense">Paid</CustomTableCell>
                          ...
                          {searchResults.map(result => {
                            ...
                            return <TableRow className={classes.row}>
                              ...
                              <CustomTableCell padding="dense">
                                {result.created ? <div>{moment(result.created).format('MMM Do YYYY')}</div> : null}
                                <br />
                                {result.created ? <div>{moment(result.created).format('h:mm a')}</div> : null}
                              </CustomTableCell>
                              <CustomTableCell padding="dense">
                                {result.scannedTime ? <div>{moment(result.scannedTime).format('MMM Do YYYY')}</div> : null}
                                <br />
                                {result.scannedTime ? <div>{moment(result.scannedTime).format('h:mm a')}</div> : null}
                              </CustomTableCell>
                                ...
                                {!result.paid ?
                                  <Button variant="contained" color="primary" onClick={() => {
                                    this.setState({
                                      ...
                                      plate: result.plate }, () => {
                                       this.setTimes(result.plate, new Date(result.created));
                                     });
                                   }}>Select
                                </Button> : ''}
                              </CustomTableCell>
